...
void print_help() {
    std::cout << "\n========== KVStore Client Commands ==========\n"
              ...
              << "  begin [level]                        - Begin transaction (default: SNAPSHOT)\n"
              << "      levels: 0=READ_UNCOMMITTED, 1=READ_COMMITTED, 2=REPEATABLE_READ, 3=SNAPSHOT_ISOLATION, 4=SERIALIZABLE\n\n"
              << "  commit                      - Commit current transaction\n"
              << "  rollback                    - Rollback current transaction\n"
              << "  set_isolation <level>       - Set isolation level\n"
              << "  get_isolation               - Get current isolation level\n"
